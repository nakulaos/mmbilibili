...
func newComment(db *gorm.DB, opts ...gen.DOOption) comment {
	...
	_comment.UID = field.NewUint(tableName, "uid")
	_comment.OwnerID = field.NewUint(tableName, "owner_id")
	_comment.OwnerType = field.NewString(tableName, "owner_type")
	_comment.Content = field.NewString(tableName, "content")
	_comment.ParentID = field.NewUint(tableName, "parent_id")
	_comment.LikeCount = field.NewInt(tableName, "like_count")
	_comment.UnLikeCount = field.NewInt(tableName, "un_like_count")
	_comment.Replies = commentHasManyReplies{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Replies", "model.Comment"),
		Replies: struct {
			field.RelationField
		}{
			RelationField: field.NewRelation("Replies.Replies", "model.Comment"),
		},
	}
...
func (c *comment) updateTableName(table string) *comment {
	...
	c.LikeCount = field.NewInt(table, "like_count")
	c.UnLikeCount = field.NewInt(table, "un_like_count")
...
func (c *comment) fillFieldMap() {
	...
	c.fieldMap["un_like_count"] = c.UnLikeCount
...
type commentHasManyReplies struct {
	db *gorm.DB
...
func (a commentHasManyReplies) Where(conds ...field.Expr) *commentHasManyReplies {
	if len(conds) == 0 {
		return &a
	}
...
func (a commentHasManyReplies) WithContext(ctx context.Context) *commentHasManyReplies {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a commentHasManyReplies) Session(session *gorm.Session) *commentHasManyReplies {
	a.db = a.db.Session(session)
	return &a
}

func (a commentHasManyReplies) Model(m *model.Comment) *commentHasManyRepliesTx {
	return &commentHasManyRepliesTx{a.db.Model(m).Association(a.Name())}
}

type commentHasManyRepliesTx struct{ tx *gorm.Association }

func (a commentHasManyRepliesTx) Find() (result []*model.Comment, err error) {
	return result, a.tx.Find(&result)
}

func (a commentHasManyRepliesTx) Append(values ...*model.Comment) (err error) {
...
func (a commentHasManyRepliesTx) Replace(values ...*model.Comment) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a commentHasManyRepliesTx) Delete(values ...*model.Comment) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a commentHasManyRepliesTx) Clear() error {
	return a.tx.Clear()
}

func (a commentHasManyRepliesTx) Count() int64 {
	return a.tx.Count()
}
