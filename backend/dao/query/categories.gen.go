...
func newCategory(db *gorm.DB, opts ...gen.DOOption) category {
	...
	_category.Type = field.NewString(tableName, "type")
	_category.Partition = field.NewString(tableName, "partition")
	...
	_category.Tags = categoryManyToManyTags{
...
func (c *category) updateTableName(table string) *category {
	...
	c.Partition = field.NewString(table, "partition")
...
func (c *category) fillFieldMap() {
	...
	c.fieldMap["partition"] = c.Partition
...
type categoryManyToManyTags struct {
	db *gorm.DB

	field.RelationField
}

func (a categoryManyToManyTags) Where(conds ...field.Expr) *categoryManyToManyTags {
	if len(conds) == 0 {
		return &a
	}
...
func (a categoryManyToManyTags) WithContext(ctx context.Context) *categoryManyToManyTags {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a categoryManyToManyTags) Session(session *gorm.Session) *categoryManyToManyTags {
	a.db = a.db.Session(session)
	return &a
}

func (a categoryManyToManyTags) Model(m *model.Category) *categoryManyToManyTagsTx {
	return &categoryManyToManyTagsTx{a.db.Model(m).Association(a.Name())}
}

type categoryManyToManyTagsTx struct{ tx *gorm.Association }

func (a categoryManyToManyTagsTx) Find() (result []*model.Tag, err error) {
	return result, a.tx.Find(&result)
}

func (a categoryManyToManyTagsTx) Append(values ...*model.Tag) (err error) {
...
func (a categoryManyToManyTagsTx) Replace(values ...*model.Tag) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a categoryManyToManyTagsTx) Delete(values ...*model.Tag) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a categoryManyToManyTagsTx) Clear() error {
	return a.tx.Clear()
}

func (a categoryManyToManyTagsTx) Count() int64 {
	return a.tx.Count()
}
